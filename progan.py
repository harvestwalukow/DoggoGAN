...
# Channel schedule (Section 6.1 - Reduced Capacity)
RESOLUTION_CHANNELS = {
    4: 512,
    8: 512,
    16: 256,   # 512 / 2
...
# 600k images per phase.
IMAGES_PER_PHASE = 400_000
...
def minibatch_stddev(x, eps=1e-8):
    """Minibatch standard deviation (Discriminator only). NVIDIA official implementation."""
    std = torch.sqrt(x.var(0, unbiased=False) + eps)
    mean_std = std.mean().view(1, 1, 1, 1)
    return torch.cat([x, mean_std.expand(x.size(0), 1, x.size(2), x.size(3))], dim=1)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        # 4x4 Block (Canonical: Dense -> Reshape -> Conv3x3)
        self.dense_4x4 = EqualizedLinear(512, 512 * 4 * 4)
        self.block_4x4 = nn.ModuleList([
            EqualizedConv2d(512, 512, 3, 1, 1),  # Conv 3x3
        ])
    ...
    def forward(self, z, step, alpha):
        ...
        # 4x4 Block (Dense -> Reshape -> Conv3x3)
        x = self.dense_4x4(z)  # (B, 512*16)
        x = F.leaky_relu(x, 0.2)
        x = x.view(-1, 512, 4, 4)  # Reshape to (B, 512, 4, 4)
        
        x = self.blocks['4'][0](x)  # Conv 3x3
        x = F.leaky_relu(x, 0.2)
        x = pixel_norm(x)
...
def train_progan(args):
    ...
    # STRICT: Load at 64, downsample recursively
    dataset_full = DogDataset(args.data_path, 64) 
    
    # Calculate total images needed for the full schedule (64x64)
    # Resolutions: 4, 8, 16, 32, 64
    # Phases: 
    #   4x4: 1 phase (stabilize)
    #   8..64: 4 transitions * 2 phases (fade + stabilize) = 8 phases
    # Total = 9 phases
    total_limit = IMAGES_PER_PHASE * 9 
    ...
    def recursive_downsample(img, current_res):
        if current_res == 64: return img
        cur = img
        steps_down = int(math.log2(64 // current_res))
        for _ in range(steps_down):
            cur = F.avg_pool2d(cur, 2)
        return cur
    
    
    def save_checkpoint(n_img, step, alpha, res):
        checkpoint_dir = "progan_checkpoints"
        os.makedirs(checkpoint_dir, exist_ok=True)
        path = os.path.join(checkpoint_dir, f"checkpoint_{n_img//1000}k.pth")
        latest_path = os.path.join(checkpoint_dir, "checkpoint_latest.pth")
        
        state = {
            'total_images_shown': n_img,
            'step': step,
            'alpha': alpha,
            'res': res,
            'gen_state_dict': (gen.module if hasattr(gen, 'module') else gen).state_dict(),
            'disc_state_dict': (disc.module if hasattr(disc, 'module') else disc).state_dict(),
    ...
    while total_images_shown < total_limit:
        ...
        for real_img_1024 in dataloader:            
            ...
            # Save Checkpoint every 50k images
            if total_images_shown % 50000 < B:
                save_checkpoint(total_images_shown, step, alpha, res)
            ...
            if (total_images_shown // 40000) > ((total_images_shown - B) // 40000):
                     ...
                     # Upscale to 64x64 for better visibility
                     if sample.shape[2] < 64:
                         sample = F.interpolate(sample, size=64, mode='nearest')
